...
import {
  Users, Search, User, Phone, Calendar, FileText,
  TestTube, ChevronDown, ChevronUp, QrCode, Pill,
  AlertCircle, BadgeCheck, Clock, X
} from "lucide-react";
...
export default function DoctorPatients() {
  ...
  const fetchPatients = useCallback(async () => {
    ...
    const { data: rxData } = await supabase
      ...
      .select(`
        *,
        patient:profiles!prescriptions_patient_id_fkey(id, full_name, phone, address)
      `)
      .eq("doctor_id", currentUser.id)
      .order("created_at", { ascending: false });

    // Get all lab requests by this doctor
    const { data: lrData } = await supabase
      .from("lab_requests")
      .select(`
        *,
        patient:profiles!lab_requests_patient_id_fkey(id, full_name, phone),
        lab_results(result_notes, uploaded_at)
      `)
      .eq("doctor_id", currentUser.id)
      .order("created_at", { ascending: false });
    ...
    (rxData || []).forEach((rx: any) => {
      ...
      if (!patientMap.has(p.id)) {
        patientMap.set(p.id, {
          id: p.id, full_name: p.full_name, phone: p.phone, address: p.address,
          prescriptionCount: 0, labCount: 0, lastVisit: null,
          prescriptions: [], labRequests: [],
        });
      }
                      ...
                      <div className="flex gap-2 mt-0.5">
                        ...
                        {patient.lastVisit && (
                          <p className="text-xs text-slate-400 flex items-center gap-1">
                            ...
                            {new Date(patient.lastVisit).toLocaleDateString("ar-DZ")}
                          </p>
                        )}
                      </div>
                    </div>
                              ...
                              <div key={rx.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-4">
                                ...
                                {rx.qr_token && (
                                  <div className="mt-2 flex items-center gap-2">
                                    <img
                                      src={`https://api.qrserver.com/v1/create-qr-code/?size=40x40&data=${rx.qr_token}`}
                                      alt="QR"
                            ...
                            {patient.labRequests.length === 0 ? (
                              ...
                              <div key={lr.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-4">
                                <div className="flex justify-between items-center mb-2">
                                  <span className="text-xs text-slate-500 flex items-center gap-1">
                                    <Calendar className="w-3 h-3" />
                                    {new Date(lr.created_at).toLocaleDateString("ar-DZ")}
                                  </span>
