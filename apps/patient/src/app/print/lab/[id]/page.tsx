...
export default function PrintLabRequest({ params }: { params: Promise<{ id: string }> }) {
      ...
      try {
        ...
        if (result.error) {
          result = await supabase
            ...
            .select(`
              *,
              patient:profiles!lab_requests_patient_id_fkey(full_name, date_of_birth, phone),
              doctor:profiles!lab_requests_doctor_id_fkey(full_name, specialty, phone, address, license_number, clinic_name)
            `)
            .eq("id", resolvedId)
            .single();
        }
        ...
        if (result.error) {
          result = await supabase.from("lab_requests").select("*").eq("id", resolvedId).single();
        }
  ...
  if (loading) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", fontFamily: "sans-serif", color: "#0056b3" }}>
      <p>Chargement de la demande d'examens...</p>
    </div>
  );
  if (!data) return (
    <div style={{ padding: 40, textAlign: "center", color: "#dc2626", fontFamily: "sans-serif" }}>
      Demande d'examens introuvable. <br/><span style={{fontSize: 12}}>{errorMsg}</span>
    </div>
  );
  ...
  const labRequestUrl = `${typeof window !== "undefined" ? window.location.origin : "https://enaya.dz"}/print/lab/${data.id}`;
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .title { text-align: center; font-size: 28px; font-weight: 800; color: #0056b3; margin: 20px 0; border-bottom: 2px solid #0056b3; display: inline-block; width: 100%; padding-bottom: 10px; }
        
        .patient-box { background: #e7f1ff; padding: 20px; border-radius: 10px; margin-bottom: 30px; display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-weight: 600; color: #333; }
        
        .exams-section { margin-top: 20px; min-height: 400px; }
        .exams-section h3 { color: #0056b3; margin-top: 0; }
        .checkbox-list { list-style: none; padding: 0; font-size: 18px; margin: 0; }
        .checkbox-list li { margin-bottom: 15px; border-bottom: 1px dashed #ccc; padding-bottom: 5px; color: #222; }
        
        .footer { margin-top: 50px; border-top: 1px solid #0056b3; padding-top: 20px; display: flex; justify-content: space-between; align-items: flex-end; }
        .note { flex: 1; }
      ...
      <div className="page" dir="ltr">
        ...
        <div className="title">Demande d'examens médicaux</div>
        ...
        <div className="exams-section">
          <h3>Veuillez effectuer les examens suivants:</h3>
          <ul className="checkbox-list">
            {(data.tests_list || []).map((test: any, idx: number) => {
        ...
        <div className="footer">
          <div className="note">
            <p><strong>Note:</strong> {data.doctor_notes || "....................................................."}</p>
            <div className="qr-section">
               <img src={`https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=${encodeURIComponent(labRequestUrl)}`} alt="QR Code" className="qr-img" />
