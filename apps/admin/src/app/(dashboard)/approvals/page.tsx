...
export default function AdminApprovals() {
  ...
  const updateStatus = async (id: string, status: string) => {
    ...
    if (user) {
      await supabase.from("audit_logs").insert([{actor_id: user.id, actor_role: "admin", action: status === "approved" ? "ACCOUNT_APPROVED" : "ACCOUNT_REJECTED", entity_type: "profiles", entity_id: id, metadata: { details: `تم ${status === "approved" ? "اعتماد" : "رفض"} الحساب` }}]);
    }
    fetchProfiles();
    setActioning(null);
  };
